 ...
 */
public class KlyphSlidingDrawer extends ViewGroup {
     ...
     */
    public KlyphSlidingDrawer(Context context, AttributeSet attrs) {
     ...
     */
    public KlyphSlidingDrawer(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.KlyphSlidingDrawer, defStyle, 0);

        int orientation = a.getInt(R.styleable.KlyphSlidingDrawer_orientation, ORIENTATION_VERTICAL);
        mVertical = orientation == ORIENTATION_VERTICAL;
        mBottomOffset = (int) a.getDimension(R.styleable.KlyphSlidingDrawer_bottomOffset, 0.0f);
        mTopOffset = (int) a.getDimension(R.styleable.KlyphSlidingDrawer_topOffset, 0.0f);
        mAllowSingleTap = a.getBoolean(R.styleable.KlyphSlidingDrawer_allowSingleTap, true);
        mAnimateOnClick = a.getBoolean(R.styleable.KlyphSlidingDrawer_animateOnClick, true);

        int handleId = a.getResourceId(R.styleable.KlyphSlidingDrawer_handle, 0);
        if (handleId == 0) {
            throw new IllegalArgumentException("The handle attribute is required and must refer "
                    + "to a valid child.");
        }

        int contentId = a.getResourceId(R.styleable.KlyphSlidingDrawer_content, 0);
        if (contentId == 0) {
    ...
    private void prepareContent() {
        ...
        if (Android.isMinAPI(11))
        	checkHardwareAccelerated(content);
    ...
    public void onInitializeAccessibilityEvent(AccessibilityEvent event) {
        ...
        event.setClassName(KlyphSlidingDrawer.class.getName());
    }

    @Override
    public void onInitializeAccessibilityNodeInfo(AccessibilityNodeInfo info) {
        super.onInitializeAccessibilityNodeInfo(info);
        info.setClassName(KlyphSlidingDrawer.class.getName());
    }
    ...
    private static class SlidingHandler extends Handler {
    	...
    	private final KlyphSlidingDrawer drawer;
    	public SlidingHandler(KlyphSlidingDrawer drawer)
    	{
    		super();
    		this.drawer = drawer;
    	}
